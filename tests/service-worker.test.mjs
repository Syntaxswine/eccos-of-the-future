...
function createWorkerHarness() {
  ...
  const caches = {
    ...
    keys: async () => ['ecco-v5-coherent-shell-2026.08.19.5', 'ecco-v6-initiation-2026.08.19.6'],
    delete: async (name) => {
...
test('the document and executable shell share one explicit release', () => {
  ...
  assert.equal(release, '2026.08.19.6');
  assert.match(swSource, /ecco-v6-initiation/u);
...
test('a returning player receives a coherent network shell despite stale cached assets', async () => {
  ...
  const scriptUrl = 'https://example.test/app.js?v=2026.08.19.6';
...
test('the coherent shell falls back offline and activation migrates an open earlier-release tab', async () => {
  ...
  await activation;
  assert.deepEqual(harness.deletedCaches, ['ecco-v5-coherent-shell-2026.08.19.5']);
  assert.equal(harness.messages[0].type, 'ECCO_SHELL_UPDATED');
  assert.equal(harness.messages[0].release, '2026.08.19.6');
