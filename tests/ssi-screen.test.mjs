...
import {
  ...
  SSI_ANSWER_DIGEST, verifyCountersign
} from '../src/ssi-screen.mjs';
import { sha256 } from '../src/ecco-core.mjs';

test('the SSI screen is solvable from its machine-readable breadcrumb', async () => {
  ...
  assert.equal((await sha256(countersign)).slice('sha256:'.length), SSI_ANSWER_DIGEST);
  assert.equal(challenge.answer_sha256, SSI_ANSWER_DIGEST);
